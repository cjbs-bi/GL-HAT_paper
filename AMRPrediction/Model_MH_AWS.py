...
# HAT-prefixed modules are shared between the genome pretraining model and the downstream predictor.
...
class HATEmbeddings(nn.Module):
    ...
    def __init__(self, config, inputs_embeds_shape = 1280):
        ...
        # Inputs already come in as ESM-2 embeddings, so project with a linear layer instead of an embedding lookup.
    ...
    def forward(
            self,
            input_ids=None,  # Kept for compatibility even though we rely on inputs_embeds.
            token_type_ids=None,
            position_ids=None,
            inputs_embeds=None,
    ):
        ...
        # Adjust here if the positional encoding strategy changes.
...
# Convert (seq_len * hidden_size) to (num_blocks * block_size * hidden_size).
...
# Convert (num_blocks * block_size * hidden_size) back to (seq_len * hidden_size).
...
class HATLayer(nn.Module):
    ...
    def forward(
        ...
        # Segment-wise encoder path: split the tokens/masks into blocks, encode, then stitch back together.
        if self.use_gene_block_encoder:
            ...
            # Restore block outputs back to flattened token layout.
        ...
        # Cross-segment encoder path: run block CLS tokens through another encoder then write them back.
        if self.use_genome_encoder:
...
@dataclass
class GeneBlockClassifierOutput(ModelOutput):
...
# Genome MLM model built on top of the hierarchical transformer.
...
class AMRPredictor(nn.Module):
    def __init__(self, genome_model, protein_embed_size = 640, genome_embed_size = 128, amr_embed_size = 1280, n_antibiotics = 53, window_size = 10, stride = 1):
        super(AMRPredictor, self).__init__()
        self.genome_model = genome_model

        for param in self.genome_model.parameters():
            param.requires_grad = False

        self.genome_fc = nn.Linear(genome_embed_size, amr_embed_size)
        self.genome_norm = nn.LayerNorm(amr_embed_size)
        self.classifier = nn.Linear(amr_embed_size, n_antibiotics)

    def forward(self, inputs_embeds, attention_mask, cluster_ids, labels = None, return_loss=True, outputs_genes = False, **kwargs):
        genome_embed = self.genome_model(
            inputs_embeds=inputs_embeds,
            attention_mask=attention_mask,
            cluster_ids=cluster_ids,
            return_dict=True,
        ...
        pretrained_embed = self.genome_norm(self.genome_fc(genome_embed))
        
        sequence_length = pretrained_embed.shape[1]
        mask = torch.ones(sequence_length, dtype=torch.bool, device=pretrained_embed.device)
        mask[::64] = False

        logits = pretrained_embed[:, mask, :]
        logit_reshaped = logits.transpose(1, 2)

        pooled_logit = F.max_pool1d(logit_reshaped, kernel_size=logit_reshaped.size(2)).squeeze(-1)
        pooled_logit = self.classifier(pooled_logit)

        loss = None
        if labels is not None:
            mask = labels >= 0
            loss_fn = nn.BCEWithLogitsLoss(reduction='none')
            loss = loss_fn(pooled_logit[mask], labels[mask]).mean()
        
        if outputs_genes:
            raise NotImplementedError("Gene-level outputs are not supported in this predictor.")
            
        return GeneBlockClassifierOutput(
            loss=loss,
            logits=pooled_logit,
            hidden_states=None,
            attentions=None,
            gene_block_attentions=None,
        )
