...
from Model_MP_nopretrain import HATConfig, HATForMaskedLM, AMRPredictor
...
embedding = np.memmap(f"/data/jupyterhub/hsoh/genomic-foundation-model/new_dataset/genomic-foundation-model/new_dataset/embed_protein.dat", dtype='float32', mode='r+', shape=(11179545, 1280))
...
def build_models(config_path: str) -> tuple:
    """
    Create the HAT masked-language model and wrap it with the AMR predictor.
    """
    config = HATConfig.from_pretrained(config_path)
    genome_model = HATForMaskedLM(config)
    amr_predictor = AMRPredictor(genome_model.hi_transformer)
    return genome_model, amr_predictor
...
for r in [1e-5]:
    ...
    if os.path.exists(output_dir_train):
        ...
        train_preprocessed_data = preprocess_and_save(train_dataset.select(range(10)), output_dir_train)
        val_preprocessed_data = preprocess_and_save(val_dataset.select(range(10)),  output_dir_valid)    

    training_args = TrainingArguments(
        output_dir=f"./results_nd2_gf_noamr_ga{g}_it{t}_lr{r}_gene_lnmpli_nopt",
        save_steps=4590,
        save_strategy='steps',
    ...
    # genome_model.load_state_dict(torch.load('./results_genome_newdata_epoch588/checkpoint-19141752/pytorch_model.bin'))
